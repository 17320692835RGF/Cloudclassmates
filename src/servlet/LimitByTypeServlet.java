...
public class LimitByTypeServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if("虚拟教室".equals(type)){
			request.getRequestDispatcher("/cShowType.jsp").forward(request, response);
		}
		else if("自习教室".equals(type)){
			request.getRequestDispatcher("/cShowType1.jsp").forward(request, response);
		}
		else{
